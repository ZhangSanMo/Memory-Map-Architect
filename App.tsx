...
const InputField: React.FC<InputFieldProps> = ({ 
  ...
  <div className={`space-y-1 relative group flex-1`}>
    <label className={`text-[10px] font-bold uppercase ml-1 ${isDerived ? 'text-cyan-600' : 'text-slate-500'}`}>
      {label} {isDerived && <span className="text-[8px] bg-cyan-950 px-1 rounded ml-1 border border-cyan-800">Calc</span>}
    </label>
    ...
    <div className="min-h-screen flex flex-col selection:bg-cyan-500/30">
      <header className="bg-slate-950 border-b border-slate-800 px-8 py-4 sticky top-0 z-50 flex justify-between items-center backdrop-blur-md bg-opacity-80">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold tracking-tight text-white">Memory Map Architect</h1>
            <p className="text-[10px] text-slate-500 font-medium uppercase tracking-[0.2em]">Manual Address Allocation</p>
          </div>
        </div>
        <div className="flex items-center gap-4">
           {nextHint !== null && (
             <div className="hidden md:block text-[10px] mono text-slate-500 bg-slate-900 border border-slate-800 px-3 py-1.5 rounded-full">
               Next Address: <span className="text-cyan-500">{toHex(nextHint)}</span>
             </div>
           )}
           <button 
      ...
      <main className="flex-1 grid grid-cols-12 gap-8 p-8 overflow-hidden max-w-[1600px] mx-auto w-full">
        <div className="col-span-12 lg:col-span-8 flex flex-col gap-6 overflow-hidden">
          <section className={`bg-slate-900 border transition-all duration-500 rounded-2xl p-6 shadow-2xl relative overflow-hidden ${hasConflict ? 'border-red-500' : editingId ? 'border-amber-500/50' : 'border-slate-800'}`}>
            ...
            <div className="flex flex-col gap-5">
              <div className="flex gap-4">
                <div className="flex-[2] space-y-1">
                  ...
                  <input ref={nameInputRef} type="text" value={newName} onChange={(e) => setNewName(e.target.value)} placeholder="e.g. FLASH_BOOT" className="w-full bg-slate-950 border border-slate-800 rounded-lg px-3 py-2 text-sm focus:border-cyan-500 outline-none transition-all text-slate-200" />
                </div>
                <div className="flex-1 space-y-1">
                  <label className="text-[10px] text-slate-500 font-bold uppercase ml-1">Type</label>
                  <select value={newType} onChange={(e) => setNewType(e.target.value as MemoryBlock['type'])} className="w-full bg-slate-950 border border-slate-800 rounded-lg px-3 py-2 text-sm focus:border-cyan-500 outline-none text-slate-200">
              ...
              <div className="flex gap-4 items-end">
                <div className="flex-1 space-y-1">
                   ...
                   <input type="text" value={newDesc} onChange={(e) => setNewDesc(e.target.value)} placeholder="Documentation text..." className="w-full bg-slate-950 border border-slate-800 rounded-lg px-3 py-2 text-sm focus:border-cyan-500 outline-none transition-all text-slate-200" />
                </div>
                <div className="flex gap-2">
                  ...
                  <button onClick={handleCommit} disabled={!canCommit} className={`h-[38px] px-8 rounded-lg font-bold transition-all flex items-center gap-2 ${canCommit ? (editingId ? 'bg-amber-500 text-slate-950' : 'bg-cyan-500 text-slate-950') : 'bg-slate-800 text-slate-600 cursor-not-allowed'}`}>
              ...
              <table className="w-full text-left border-collapse">
                ...
                <tbody className="divide-y divide-slate-800/50">
                  {blocks.length > 0 ? (
                    blocks.map(block => (
                      <tr key={block.id} className={`group hover:bg-slate-800/40 transition-colors ${editingId === block.id ? 'bg-cyan-900/10' : ''}`}>
                        <td className="p-4">
                          ...
                          <div className="text-[10px] text-slate-600 mt-1">{block.description}</div>
                        </td>
                    ...
                    <tr><td colSpan={4} className="p-20 text-center text-slate-700 italic text-sm">No memory regions defined yet.</td></tr>
                  )}
                </tbody>
              </table>
            </div>
